...
function save_category(array $data, ?int $id = null): int {
    if ($id) {
        db_query("UPDATE categories SET name=?,name_np=?,slug=?,color=?,icon=?,sort_order=? WHERE id=?",
            [$data['name'],$data['name_np'],$data['slug'],$data['color'],$data['icon'],$data['sort_order'],$id]);
        return $id;
    }
    return db_insert("INSERT INTO categories (name,name_np,slug,color,icon,sort_order) VALUES (?,?,?,?,?,?)",
...
function save_author(array $data, ?int $id = null): int {
    if ($id) {
        db_query("UPDATE authors SET name=?,name_np=?,slug=?,bio=?,avatar_url=? WHERE id=?",
            [$data['name'],$data['name_np'],$data['slug'],$data['bio'],$data['avatar_url'],$id]);
        return $id;
    }
    return db_insert("INSERT INTO authors (name,name_np,slug,bio,avatar_url) VALUES (?,?,?,?,?)",
...
function save_tag(array $data, ?int $id = null): int {
    ...
    return db_insert("INSERT OR IGNORE INTO tags (name,slug) VALUES (?,?)", [$data['name'],$data['slug']]);
}
...
function save_article(array $data, ?int $id = null): int {
    ...
    if (!empty($data['tag_ids'])) {
        $ts = $db ?? get_db();
        $st = $ts->prepare("INSERT OR IGNORE INTO article_tags (article_id,tag_id) VALUES (?,?)");
...
function get_active_ads(string $position): array {
    return db_fetchAll(
        "SELECT * FROM advertisements WHERE position=? AND active=1 ORDER BY sort_order",
        [$position]);
}
...
function save_ad(array $data, ?int $id = null): int {
    ...
    $cols = implode(',', $fields);
    $phs  = implode(',', array_fill(0, count($fields), '?'));
    return db_insert("INSERT INTO advertisements ($cols) VALUES ($phs)",
...
function get_events(array $opts = []): array {
    ...
    return db_fetchAll("SELECT * FROM events WHERE $ws ORDER BY $order LIMIT $limit OFFSET $offset", $params);
}
...
function save_event(array $data, ?int $id = null): int {
    ...
    $cols = implode(',', $fields);
    $phs  = implode(',', array_fill(0, count($fields), '?'));
    return db_insert("INSERT INTO events ($cols) VALUES ($phs)",
        array_map(fn($f) => $data[$f] ?? null, $fields));
}
...
function save_static_page(array $data, ?int $id = null): int {
    ...
    $cols = implode(',', $fields);
    $phs  = implode(',', array_fill(0, count($fields), '?'));
    return db_insert("INSERT INTO static_pages ($cols) VALUES ($phs)",
        array_map(fn($f) => $data[$f] ?? null, $fields));
}
...
function save_newsletter_email(string $email, string $name = ''): bool {
    ...
    try {
        db_query("INSERT OR IGNORE INTO newsletter_subscribers (email,name) VALUES (?,?)", [$email, $name]);
        return true;
    } catch (Exception $e) { return false; }
}
function get_subscribers(int $limit = 100, int $offset = 0): array {
    return db_fetchAll("SELECT * FROM newsletter_subscribers ORDER BY created_at DESC LIMIT ? OFFSET ?", [$limit, $offset]);
}
